...
function jhdup_perform_deactivation_of_current_user()
{
    ...
    $new_data = ['user_login'        => $current_user->user_login.$dectivation_suffix,
                        'user_email' => $current_user->user_email.$dectivation_suffix,
                     ];

    $wpdb->update($wpdb->users, $new_data, ['ID' => $current_user->ID]);

    $wpdb->update($wpdb->posts, ['post_status' => 'draft'], ['post_author' => $current_user->ID]);
